...
export function useSocket(options: UseSocketOptions | null) {
    ...
    useEffect(() => {
        ...
        socket.on('player_typing', (data: { userId: string; letterCount: number }) => {
            setPlayers((prev) => {
                const next = new Map(prev);
                const player = next.get(data.userId);
                if (player) {
                    next.set(data.userId, { ...player, letterCount: data.letterCount });
                }
                return next;
            });
        });

        socket.on('player_guess', (data: { userId: string; row: LetterState[]; gameStatus: string }) => {
            setPlayers((prev) => {
                const next = new Map(prev);
                const player = next.get(data.userId);
                if (player) {
